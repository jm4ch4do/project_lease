...
def credit_card_list(request):
    ...
    if not request.user.is_authenticated:
        return Response({'response': "Logging to get credit card list"},
                        status.HTTP_401_UNAUTHORIZED)

    # extra permission: user can't modify password if he's inactive
    if not request.user.is_active:
        return Response({'response': "No permission to get credict card list"},
                        status.HTTP_401_UNAUTHORIZED)

    customers = CreditCard.objects.all()
    serializer = CreditCardSerializer(customers, many=True)
...
def credit_card_edit(request, pk):
    ...
    if request.user != target_credit_card.user and \
...
def credit_card_search(request):
    ...
    queryset = CreditCard.objects.filter(status=1)
